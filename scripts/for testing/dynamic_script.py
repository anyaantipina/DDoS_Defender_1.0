...
def SwitchOff(switch, net):
    ...
    for link in net.links:
        ...
        if (link.intf1.node.name == sw_name) | (link.intf2.node.name == sw_name):
            links.append(link)
    for link in links:
        print(link.intf1.node.name, link.intf2.node.name, "down ?")
    ...
    for hostName in net.topo.hosts():
        ...
        if pid == 0:
            ...
            if hostName == "h1":
                ...
                print(hostName, host.cmd("./runos/src/apps/ddos-defender/scripts/png.sh %s %s" % (pingPair.IP(),interval)))
                ...
                result = host.cmd("./runos/src/apps/ddos-defender/scripts/atck.sh %s %s 25" % (host.MAC(), host.IP()))
                curr_time = monotonic() - start
                ...
                handle.write("'%s' : [%s, %s]" % (host.IP(), start_attack, end_attack))
                handle.close()
            ...
            if hostName == "h2":
                ...
                print("curr_time=%s :: %s start attack" % (curr_time,host.IP()))
                start_attack = curr_time
                result = host.cmd("./runos/src/apps/ddos-defender/scripts/atck.sh %s %s 35" % (host.MAC(), host.IP()))
                curr_time = monotonic() - start
                end_attack = curr_time
                print("curr_time=%s :: %s end attack" % (curr_time, host.IP()))
            ...
            if hostName == "h8":
                ...
                print(hostName, host.cmd("./runos/src/apps/ddos-defender/scripts/png.sh 10.0.0.20 %s" % interval))
                curr_time = monotonic() - start
...
"""
    ...
    if sleep_time > 0:
        sleep(sleep_time)
    curr_time = monotonic() - start
    print('"curr_time=%s :: switch s3 off' % curr_time)
    ...
    if sleep_time > 0:
        sleep(sleep_time)
    curr_time = monotonic() - start
    print('"curr_time=%s :: switch s3 up' % curr_time)
