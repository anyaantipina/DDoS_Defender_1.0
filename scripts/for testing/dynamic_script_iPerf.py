...
def simpleTest():
    ...
    for hostName in net.topo.hosts():
        ...
        if pid == 0:
            ...
            if hostName == "h3":
                ...
                curr_time = monotonic() - start
                print("curr_time=%s :: %s start iperf to server %s for 25 sec" % (curr_time, host.IP(), pingPair.IP()))
                result = host.cmd("iperf3 -c %s -t 25" % pingPair.IP())
                handle = open("outputH3(c)-H9(s).txt", "w")
                handle.write(result)
                handle.close()
            ...
            if hostName == "h4":
                ...
                curr_time = monotonic() - start
                interval = (9 - curr_time) if (9 - curr_time) >=1 else 1
                print("curr_time=%s :: %s start ping %s for %s sec" % (curr_time, host.IP(), pingPair.IP(), interval))
                print(hostName, host.cmd("./png.sh %s %s" % (pingPair.IP(),interval)))
                ...
                print("curr_time=%s :: %s start iperf to server %s for 25 sec" % (curr_time, host.IP(), pingPair.IP()))
                result = host.cmd("iperf3 -c %s -t 25" % pingPair.IP())
                handle = open("outputH4(c)-H6(s).txt", "w")
                handle.write(result)
                handle.close()
            ...
            if hostName == "h6":
                ...
                curr_time = monotonic() - start
                print("curr_time=%s :: %s start iperf server" % (curr_time, host.IP()))
                print(hostName, host.cmd("iperf3 -s -1"))
            ...
            if hostName == "h8":
                pingPair = net.get("h9")
                curr_time = monotonic() - start
                interval = (10 - curr_time) if (10 - curr_time) >=1 else 1
                print("curr_time=%s :: %s start ping %s for %s sec" % (curr_time, host.IP(), pingPair.IP(), interval))
                print(hostName, host.cmd("./png.sh %s %s" % (pingPair.IP(),interval)))
    ...
    handleResult = open("outputIPerf.txt", "w")
    handle = open("outputH3(c)-H9(s).txt", "r")
    result = handle.read()
    handleResult.write(result)
    handle.close()
    handle = open("outputH4(c)-H6(s).txt", "r")
    result = handle.read()
    handleResult.write(result)
    handle.close()
    handleResult.close()
    ...
    if pid == 0:
        host = net.get("h1")
        print(host.cmd("sudo rm 'outputH3(c)-H9(s).txt' 'outputH4(c)-H6(s).txt'"))
        sys.exit(3)
    pid, status = os.wait()
